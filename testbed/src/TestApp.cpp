...
int TestApp::Init()
{
	...
	if (glewInit() != GLEW_OK)
		std::cout << "Testbed Error - TestApp::glewInit() != GLEW_OK" << std::endl;
	std::cout << "Testbed Log - TestApp::" << glGetString(GL_VERSION) << std::endl;
...
void TestApp::LoadScene(unsigned int index)
{
	...
	case 1:
	{
		...
		if (m_solver.CreateOrientedBox(handle, Vector2(2, 2), Vector2(0, -2), 44 * PIP_DEG2RAD, Vector2(0, 0), 0.0f, 10.f, 0.7f) != -1) m_bodyHandles.push_back(handle);
